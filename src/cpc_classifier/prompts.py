...
CLASSIFIER_SYSTEM = (
    ...
    "- For each chosen class, give a confidence (0-1) and an evidence_span: a short VERBATIM "
    "quote from the INVENTION description that supports it. Copy it exactly — do not paraphrase; "
    "spans that are not verbatim substrings of the description are discarded.\n"
    "- Select ONLY classes the description explicitly supports. Do not infer mechanisms the text "
    "does not state (e.g. do not add security or cryptographic classes because a payment is "
    "'authorised', or public-key classes when the text does not describe public-key cryptography). "
    "Prefer fewer, well-supported classes over broad coverage.\n"
